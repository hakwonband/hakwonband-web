 ...
 */
(function($) {
	jQuery.fn.html5_upload = function(customOptions) {
		function get_file_name(file) {
			return encodeURIComponent(file.name || file.fileName);
		}
		...
		var options = jQuery.extend({
			...
			sendBoundary: false,
			fieldName: 'uploadFiles',//ignore if sendBoundary is false
			...
			function upload_file(number) {
				...
				if (!options.sendBoundary) {
					...
					if (window.FormData) {//Many thanks to scottt.tw
						...
						if( options.customExtraFields ) {
							options.extraFields = options.customExtraFields;
						}
...
UploadOptions.prototype = {
	url : uploadUrl
	, sendBoundary : window.FormData || $.browser.mozilla
	, onFinishOne : function(event, response, name, number, total) {
		try {
			var responseJson = JSON.parse(response);
			if( responseJson.colData && responseJson.colData.fileNo > 0 ) {
				this.uploadFileArray.push(responseJson.colData);
			} else {
				//	실패
				this.errorCount++;
			}
 ...
 * 학원 엑셀 업로드
 */
var ExcelUploadOptions = function() {
};
ExcelUploadOptions.prototype = {
	url : contextPath+'/admin/upload/excel.do'
	, sendBoundary : window.FormData || $.browser.mozilla
	, onFinishOne : function(event, response, name, number, total) {
		try {
			var responseJson = JSON.parse(response);
			if( responseJson.colData ) {
				this.uploadResult = responseJson.colData;
			} else {
				//	실패
				this.errorCount++;
			}
	...
	, onError : function(event, name, error) {
		alert('error while uploading excel file ' + name);
	}
	, onStart : function(event, total) {
		this.errorCount = 0;
		this.errorFileArray = [];
