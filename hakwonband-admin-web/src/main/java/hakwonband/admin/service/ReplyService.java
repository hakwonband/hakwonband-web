...
public class ReplyService {
	...
	public DataMap registReply(DataMap param) {
		...
		if( param.equals("content_type", "002") ) {
			...
			String reciveUserNo = "";
			String reciveUserType = "";
			/*	보낸 사람이면 받는 사람에게/받는 사람이면 보낸 사람에게	*/
			if( messageUserInfo.equals("receive_user_no", param.getString("reg_user_no")) ) {
				reciveUserNo = messageUserInfo.getString("send_user_no");
				reciveUserType = messageUserInfo.getString("send_user_type");
			} else if( messageUserInfo.equals("send_user_no", param.getString("reg_user_no")) ) {
				reciveUserNo = messageUserInfo.getString("receive_user_no");
				reciveUserType = messageUserInfo.getString("receive_user_type");
			}
			if( StringUtils.isBlank(reciveUserNo) ) {
				throw new HKBandException("registReply reciveUserNo is null");
			}

			messageParam.put("reciveUserNo", reciveUserNo);
			...
			if( deviceList != null && deviceList.size() > 0 ) {
				...
				if( HakwonConstant.UserType.ADMIN.equals(reciveUserType) ) {
					pushMessage.setLink_url("https://admin.hakwonband.com/main.do#/message/sendSingleMessageDetail?receiveNo="+receiveNo);
				} else if( HakwonConstant.UserType.WONJANG.equals(reciveUserType) || HakwonConstant.UserType.TEACHER.equals(reciveUserType) ) {
				...
				} else if( HakwonConstant.UserType.STUDENT.equals(reciveUserType) || HakwonConstant.UserType.PARENT.equals(reciveUserType) ) {
					pushMessage.setLink_url("https://m.hakwonband.com/index.do#/messageDetail?receive_no="+receiveNo+"&type=receive");
				}
