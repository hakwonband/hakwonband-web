...
hakwonApp.controller('hakwonSearchController', function($scope, $location, $routeParams, hakwonService, CommUtil) {
	...
	/**
	 * 5초가 될때 까지 돌다가 5초 이상일때는 중지 한다.
	 */
	var location_watch = setInterval(function() {
		console.log('location_watch call', gpsLocation);

		var address = comm.getAddress();
		if( gpsLocation.latitude && gpsLocation.longitude && address ) {
			$scope.setInit();
		}
	}, 500);
	setTimeout(function(){
