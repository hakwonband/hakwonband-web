...
//			return;
		...
		/*  선생님 가입단계중 '03'번 단계에서만 학원정보 체크  */
		if ($scope.currentStep == '03' && $scope.regUserInfo.user_type == '004') {
			if (isNull($scope.regUserInfo.hakwon_no) || isNull($scope.regUserInfo.hakwon_code)) {
				alert('학원 정보가 올바르지 않습니다.');
				$scope.searchHakwon = 'false';
				return;
			}

			if ($scope.searchHakwon == 'false') {
				alert('학원을 검색해 주세요.');
				return;
			}
		}
...
hakwonCommonApp.controller('SignUpController', function($scope, $location, SignUpService, CommUtil) {
	...
	try {
		...
		/*  가입단계 이동 구현 */
		$scope.moveFirstStep = function() {
			$scope.currentStep = '01';
		};

		$scope.moveSecondStep = function() {
			$scope.currentStep = '02';
		};

		$scope.moveThirdStep = function() {
			console.log($scope.regUserInfo);

			if ($scope.regUserInfo.user_type == '004') {
				if (SignUpService.validateUserInfo($scope)) {
					$scope.currentStep = '03';
				}
			} else {
		...
		/*	입력키에 따라 학원코드 체크 초기화	*/
		$scope.resetHakwonCodeCheck = function(e) {
			if ($scope.searchHakwon != 'true' && e.keyCode === 13) {
				$scope.searchHakwonInfo();
				return;
			}
		...
		/*  학원코드로 학원조회  */
		$scope.searchHakwonInfo = function() {
			console.log($scope.regUserInfo);
			if (!$scope.regUserInfo.hakwon_code) {
				alert('학원코드를 입력해주세요.');
				$scope.searchHakwon = 'false';
				return ;
			}
			var params = {hakwon_code : $scope.regUserInfo.hakwon_code};
			CommUtil.ajax({param:params, url:contextPath+"/searchHakwonCode.do", successFun:function(data) {
				try {
					if (data.colData) {
						if (data.colData.result == 'notexist') {
							$scope.searchHakwon = 'false';
						} else if (data.colData.result == 'exist') {
							$scope.searchHakwon = 'true';
							$scope.regUserInfo.hakwon_no		= data.colData.hakwonInfo.hakwon_no;
							$scope.regUserInfo.hakwon_name		= data.colData.hakwonInfo.hakwon_name;
							$scope.regUserInfo.master_user_name = data.colData.hakwonInfo.master_user_name;
						} else {
							commProto.logger({duplicateCheckIdError:data});
						}
					} else {
		...
		$scope.registUser = function() {
			CommUtil.ajax({param:$scope.regUserInfo, url:contextPath+"/registHakwonUser.do", successFun:function(data) {
				try {
					if (data.colData.registUser == CommonConstant.Flag.success) {
						$scope.currentStep = '04';
						if( $scope.regUserInfo.user_type == '003' ) {
							/*	원장님은 가입 후 바로 로그인 페이지로 이동	*/
