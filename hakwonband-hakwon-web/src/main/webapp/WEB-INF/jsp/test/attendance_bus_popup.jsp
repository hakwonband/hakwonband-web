	...
	<div class="ibox attendance_popup">
		...
		<div class="ibox-content">
			...
			<form id="attendanceForm" name="attendanceForm" onsubmit="return false;">
				...
				<input type="hidden" name="dataType" value="002">
			...
			<button class="btn btn-info dim btn_submit" type="button" action-type="start">승차</button>
			<button class="btn btn-info dim btn_submit" type="button" action-type="end">하차</button>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("div.attendance_popup div.ibox-content button.btn_submit").click(function(){
			...
			if("start" == type) {
				if(confirm(stName+ "님, 승차 하시겠습니까?")) {
					busStart();
				}
			} else {
				if(confirm(stName + "님, 하차 하시겠습니까?")) {
					busEnd();
				}
			}
		});
	...
	function busStart(queryString) {
		$("input[name=attendanceType]").val("start");
		
		$.ajax({
			url: "<%=contextPath%>/hakwon/attendance/bus.do",
			type: "post",
			data: $("#attendanceForm").serialize(),
			dataType: "json",
			success: function(data) {
				...
				if(data.colData) {
					if("success" == data.colData.flag) {
						alert($("input[name=attendanceCode]").val() + "번 승차가 완료되었습니다.");
	...
	function busEnd(queryString) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.colData) {
					if("success" == data.colData.flag) {
						alert($("input[name=attendanceCode]").val() + "번 하차가 완료되었습니다.");
