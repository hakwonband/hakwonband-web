...
import hakwonband.hakwon.service.SignUpService;
...
public class SignUpController extends BaseAction {
	...
	/**
	 * 선생님 회원가입시, 학원코드로 학원검색
	 * @param request
	 * @param response
	 */
	@RequestMapping("/searchHakwonCode")
	public void searchHakwonCode(HttpServletRequest request, HttpServletResponse response) {
		String hakwon_code = request.getParameter("hakwon_code");

		DataMap param = new DataMap();
		param.put("hakwon_code", hakwon_code);

		DataMap resultObj = signupService.searchHakwonCode(param);
		sendColData(resultObj, request, response);
	}
	 ...
	 * @param response
	 */
	@RequestMapping("/registHakwonUser")
	public void registTeacher(HttpServletRequest request, HttpServletResponse response) {
		...
		/* 사용자 등록 */
		DataMap colData = signupService.registHakwonUser(param);
