package hakwonband.hakwon.common.exception;
...
public class GeneralExceptionResolver extends SimpleMappingExceptionResolver {
	...
	public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
		...
		if( ex instanceof LoginFailException ) {
			...
			errorMessage = ex.getMessage();
			logging = false;
		} else if( ex instanceof RoleFailException ) {
			flag = "roleFail";
			errorReqType = "RoleFailException";
			errorMessage = ex.getMessage();
			logging = false;
		} else {
...
//			response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
			...
			/*	direct로 로그인 페이지로 이동 가능	*/
			if( ex instanceof LoginFailException ) {
				modelAndView = new ModelAndView("redirect:/index.do");

			} else if( ex instanceof NoLoginFailException ) {
				modelAndView = new ModelAndView("redirect:/hakwon/main.do");
			} else {
